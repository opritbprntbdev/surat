    ...
    <header class="header">
        ...
        <div class="header-center">
            <div class="search-container">
                ...
                <input type="text" id="page-search-input" placeholder="Cari surat terkirim..." class="search-input">
            </div>
...
(function(){
                ...
                try{
                    ...
                    try {
                        ...
                        if (window.App && typeof App.refreshUnreadBadge === 'function') {
                            ...
                            // Fallback: update badge langsung
    ...
    function renderPager(){
        ...
        if (prev) prev.onclick = ()=>{ if (page>1){ page--; loadSent(); } };
        if (next) next.onclick = ()=>{ if (page<pages){ page++; loadSent(); } };
    ...
    async function loadSent(){
        ...
        try{
            ...
            const resp = await API.getSuratList({ box: 'sent', q, page, page_size: pageSize });
            rows = resp.data?.data || [];
    ...
    if (searchInput){
        const deb = Utils.debounce(()=>{ page = 1; loadSent(); }, 200);
        ['input','keyup','change'].forEach(evt => searchInput.addEventListener(evt, deb));
    }

    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', loadSent); else loadSent();
})();
</script>
